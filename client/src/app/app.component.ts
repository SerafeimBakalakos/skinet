...
export class AppComponent implements OnInit {
  title = 'Skinet';
  products: any[] = []; // initial value is necessary when using strict mode in TS
  ...
  ngOnInit(): void {
    ...
    this.http.get('https://localhost:5001/api/products?pageSize=50').subscribe({
      next: (response: any) => this.products = response.data, // what to do after the response is received
